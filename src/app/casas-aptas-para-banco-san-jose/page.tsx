...
const LANDING_SLUG = 'casas-aptas-para-banco-san-jose';
...
export default async function CasasAptasParaBancoSanJosePage() {
  const landing = getLevelBLandingBySlug(LANDING_SLUG);
  if (!landing) notFound();
  ...
  const fallbackProperties = allProperties.filter(
    (p) => p.status !== 'retirada' && p.status !== 'inactiva' && p.category === 'casa'
  );
