...
export function SeoEditorSection({
  ...
  // Autogeneración de Slug estándar
  const autoGeneratedSlug = generatePropertySlug({
  ...
  const effectiveSlug = slug && slug.trim() ? slug.trim() : autoGeneratedSlug;
  ...
  const handleForceGoogleIndexing = async () => {
    ...
    try {
      ...
      const targetUrl = `${siteOrigin}/propiedad/${effectiveSlug}`;
  ...
  const searchConsoleInspectUrl = `https://search.google.com/search-console/inspect?resource_id=${encodeURIComponent('https://inmobiliariamontano.uy')}&url=${encodeURIComponent(`https://inmobiliariamontano.uy/propiedad/${effectiveSlug}`)}`;
    ...
    <div className="space-y-6 pt-6 border-t border-slate-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-3 gap-2">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium">
            Control de URLs limpias, títulos persuasivos, indexación y snippets para posicionar #1 en Google San José.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          {/* Campo 0: Slug Personalizable (URL Amigable Canónica) */}
          <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-slate-800 uppercase flex items-center space-x-1.5">
                <span>URL Amigable / Slug Canónico</span>
              </label>
              
              <button
                type="button"
                onClick={handleResetSlug}
                className="text-[11px] font-bold text-[#5E1754] hover:text-[#E85D04] flex items-center space-x-1 transition-colors"
                title="Regenerar slug automáticamente según la regla estándar"
              >
            ...
            <div className="flex items-center bg-white border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-[#5E1754]">
              <span className="bg-slate-100 px-3 py-2.5 text-[11px] sm:text-xs text-slate-500 font-mono border-r border-slate-200 select-none flex-shrink-0">
                /propiedad/
              </span>
              <input
                type="text"
                ...
                placeholder={autoGeneratedSlug}
                className="w-full bg-transparent px-3 py-2 text-xs sm:text-sm font-mono text-slate-900 focus:outline-none"
              />
            </div>
            <p className="text-[11px] text-slate-500 font-medium">
              Estructura estándar: <code className="text-[10px] bg-slate-200 px-1 py-0.5 rounded text-slate-700">[operacion]-[tipo]-[caracteristica]-[ubicacion]-[ciudad]-[id]</code>. Si lo modificas, el enlace anterior redirigirá automáticamente sin dar error 404.
            </p>
          </div>
          ...
          <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 space-y-2">
            ...
            />
            <p className="text-[11px] text-slate-500 font-medium">
              Texto persuasivo en los resultados de búsqueda. Ideal entre 130 y 155 caracteres con llamada a Daniel Montaño al 092 776 715.
            </p>
          </div>

          {/* Campo 3: Palabras Clave (Focus Keywords) */}
          <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 space-y-2">
            <label className="text-xs font-bold text-slate-800 uppercase flex items-center space-x-1.5">
              <span>Palabras Clave (Focus Keywords)</span>
            </label>
            <input
              type="text"
              value={focusKeywords || ''}
              onChange={(e) => setFocusKeywords?.(e.target.value)}
              placeholder="ej. casa en venta san jose, plaza arriaga, apta banco, inmobiliaria montaño"
              className="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs sm:text-sm font-medium focus:ring-2 focus:ring-[#5E1754] text-slate-900"
            />
            <p className="text-[11px] text-slate-500 font-medium">
              Términos de búsqueda principales separados por comas para posicionamiento orgánico.
            </p>
          </div>

          {/* Campo 4: Estado de Indexación (Robots Meta) */}
          <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 space-y-2.5">
            <label className="text-xs font-bold text-slate-800 uppercase flex items-center space-x-1.5">
              <span>Estado de Indexación (Robots Meta)</span>
            </label>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <button
                ...
                className={`p-3 rounded-xl border flex flex-col items-start space-y-1 transition-all text-left ${
                  !noIndex
              ...
              >
                ...
                <span className="text-[11px] font-normal text-slate-500">
                  Visible e indexable en Google (Recomendado para inmuebles activos).
                </span>
              </button>
              ...
              >
                ...
                <span className="text-[11px] font-normal text-slate-500">
                  Ocultar de motores de búsqueda (Útil para inmuebles reservados o pausados).
                </span>
              </button>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4 space-y-2 shadow-2xs">
            ...
            <div className="space-y-1 pt-1">
              <div className="flex items-center space-x-1.5 text-[11px] text-slate-700 truncate">
                ...
                <span className="truncate">inmobiliariamontano.uy › propiedad › {effectiveSlug}</span>
              </div>
